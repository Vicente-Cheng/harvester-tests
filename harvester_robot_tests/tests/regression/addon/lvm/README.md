...
`LVM_TEST_RUN_ID` labels the selected BlockDevices so the cleanup stage, which runs
in a separate Pabot process, can find the disks chosen by the preparation stage. It
defaults to `lvm-robot`; override it only when separate LVM runs need distinct IDs.
