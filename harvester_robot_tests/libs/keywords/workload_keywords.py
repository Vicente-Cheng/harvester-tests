...
class workload_keywords:
    ...
    def create_workload_pod(self, pod_name, pvc_name, volume_mode="Block"):
        ...
        at /dev/test-volume, Filesystem mounts it at /data.
        """
    ...
    def write_block_pattern(self, pod_name, size_mb=128):
        ...
        self._exec(pod_name, (
            ...
            f"dd if=/dev/zero of={dev} bs=1M seek={quarter} count={2 * quarter} 2>/dev/null && "
            f"dd if=/dev/urandom of={dev} bs=1M seek={3 * quarter} count={size_mb - 3 * quarter} 2>/dev/null && "
            "sync"
        ))
